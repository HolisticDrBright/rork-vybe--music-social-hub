...
struct EarningsTransparencyView: View {
        ...
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Artist keeps 90%")
                    ...
                    .foregroundStyle(VYBE.text)
                Text("On VYBE, artists keep $0.90 of every dollar fans give. Spotify pays artists ~$0.004 per stream. The math isn't complicated.")
                    .font(.system(size: 13, weight: .medium))
                    .foregroundStyle(VYBE.textSecondary)
                    .lineSpacing(3)
            }
    ...
    private var breakdownSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            earningsRow("dollarsign.circle.fill", "Total (this month)", earnings.total, VYBE.text, bold: true)
        }
        .padding(16)
        .vybeCard(corner: 20)
    }
